...
router.post("/reels", async (req, res): Promise<void> => {
  ...
  const [stats] = await db
    .select({
      ...
      avgSavesPct: avg(reelsTable.savesPct),
    })
    .from(reelsTable);
...
router.get("/reels/stats", async (_req, res): Promise<void> => {
  const [stats] = await db
    .select({
      totalReels: count(reelsTable.id),
      avgViews: avg(reelsTable.views),
      avgSavesPct: avg(reelsTable.savesPct),
      avgLikesPct: avg(reelsTable.likesPct),
    })
    .from(reelsTable);
